#!/usr/bin/env python3
"""Evaluate Markov-state MLP-predictor LE-WM latent rollouts on one OGBench cube RGB-D trajectory."""
...
DEFAULT_DATASET_PATH = "ogbench_cube/data/test_data_depth/ogbench_cube_test_depth.h5"
DEFAULT_MODEL_DIR = "ogbench_cube/models/mlpdyn_depth"
DEFAULT_OUT_DIR = "ogbench_cube/eval/mlpdyn_depth_eval"
...
def load_episode(
    ...
    *,
    ...
    with h5py.File(dataset_path, "r") as h5:
        ...
        depth_np = np.asarray(h5["depth"][rows], dtype=np.float32)
...
def plot_latents(
    ...
    *,
    ...
    for plot_idx, (start_dim, end_dim) in enumerate(splits, start=1):
        ...
        fig.suptitle(f"Markov MLP LE-WM RGB-D latent rollout episode {episode_idx}, dims {start_dim}-{end_dim - 1}")
        fig.tight_layout()
