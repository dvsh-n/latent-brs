...
def main():
    ...
    controller = GenericMPC(
        ...
        SQPConfig(max_sqp_iterations=1, warm_start=False, feas_tol=1e-2, step_tol=1e-4, line_search=True),
    ...
    for _ in mpc_pbar:
        ...
        obs_free, gripper_height, _ = true_gripper_height_safety_status(
            current_info, cfg.gripper_height_threshold
        )
        mpc_pbar.set_postfix(
            latent_err=f"{lat_err:.4f}",
            obs_free=obs_free,
            gripper_z=f"{gripper_height:.4f}",
            status=status,
        )
