...
def make_mppi_rollout_and_eval(dynamics_fn, W_mppi, goal_state):
    ...
    # FIXED: Added trailing catch-all signature options to support third-party tracking calls
...
def main():
    ...
    # dynamics = lambda x, u, t=0.0, p=1.0: eqx_dyn(jnp.concatenate([x, u], axis=-1))
    ...
    # Fetch initial/goal image sequences 
    env.reset(seed=cfg.seed); env.unwrapped._data.qpos[:qpos_goal.shape[0]] = qpos_goal.astype(np.float64); env.unwrapped._data.qvel[:qvel_goal.shape[0]] = qvel_goal.astype(np.float64)
    goal_frame = np.asarray(env.unwrapped.render(camera="front_pixels"), dtype=np.uint8)

    env.reset(seed=cfg.seed); env.unwrapped._data.qpos[:qpos_init.shape[0]] = qpos_init.astype(np.float64); env.unwrapped._data.qvel[:qvel_init.shape[0]] = qvel_init.astype(np.float64)
    ...
    # Dynamics wrapper passed directly to maintain expected mathematical parameters
    ...
    # Filter JIT dynamically partitions the state arrays from static objects
    @eqx.filter_jit
    ...
    # Instantiate with distinct states and matrices
    ...
    for _ in mpc_pbar:
        ...
        prev_U = U_pred
        # u_raw = ((np.asarray(u0, dtype=np.float32) * train_stats.action_std) + train_stats.action_mean).astype(np.float32)
