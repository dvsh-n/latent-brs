#!/usr/bin/env python3
"""Plan in OGBench cube pixel space using Conformal SLS MPC with a fixed post-oracle grasp command."""
...
class PlanSLSOGBenchConfig:
    ...
    out_dir: Path = field(default=Path("ogbench_cube/plan/sls_mpc_conformal_fixed_grasp"))
...
def reduce_action_np(action: np.ndarray, grip_idx: int = 4) -> np.ndarray:
    action = np.asarray(action, dtype=np.float64)
    return np.concatenate([action[..., :grip_idx], action[..., grip_idx + 1 :]], axis=-1)


def resolve_fixed_grasp_action(
    cfg: PlanSLSOGBenchConfig,
    *,
...
def append_state_log(log_path: Path, *, step: int, state: np.ndarray, latent_err: float, status: str) -> None:
...
def main():
    ...
    full_dynamics = build_jax_dynamics(model.predictor.net, device, state_dim, action_dim)
    obstacle_model = None
    ...
    for _ in oracle_pbar:
        ...
        last_oracle_action_raw = raw_action.copy()
    ...
    min_state, max_state = min_state.at[:state_dim // 2].set(-0.50), max_state.at[:state_dim // 2].set(0.50)
    state_box = make_state_box_constraints(min_state, max_state)

    constraints_all = combine_constraints(state_box,
        make_control_box_constraints(-2.0*jnp.ones(control_dim), 2.0*jnp.ones(control_dim)),
    ...
    controller = GenericMPC(
        ...
        config=MPCConfig(n=state_dim, nu=control_dim, N=cfg.horizon, W=W_stage, u_ref=jnp.zeros(control_dim), dt=1.0/20.0),
        dynamics=dynamics, constraints=constraints_all,
        obstacles=jnp.zeros((0, 3)), cost=cost, num_constraints=2 * control_dim + 2 * state_dim + (1 if obstacle_constraint is not None else 0), disturbance=disturbance, shift=1,
        X_in=jnp.zeros((cfg.horizon + 1, state_dim), dtype=jnp.float64), U_in=jnp.zeros((cfg.horizon, control_dim), dtype=jnp.float64)
    )
    ...
    for step_idx in mpc_pbar:
        ...
        append_state_log(state_log_path, step=step_idx, state=current_state, latent_err=lat_err, status=status)
