#!/usr/bin/env python3
"""Evaluate Koopman (HVOK) predictor LE-WM latent rollouts on one Reacher trajectory."""
...
from shared.models import KoopmanPredictor
from train.lewm_train_koopman import LeWMReacherDataset

DEFAULT_DATASET_PATH = "data/expert_data/reacher_expert.h5"
DEFAULT_CHECKPOINT = "models/lewm_reacher_koopman/lewm_koopman_epoch_50_object.ckpt"
DEFAULT_OUT_DIR = "eval/lewm_koopman_eval"
...
def rollout_latents(
    ...
    *,
    ...
    if isinstance(model.predictor, KoopmanPredictor):
        return _rollout_koopman(
            model, true_latents, actions,
            history_size=history_size, frameskip=frameskip,
            max_rollout_steps=max_rollout_steps,
        )
    raise TypeError(f"Expected KoopmanPredictor, got {type(model.predictor).__name__}")
...
def _rollout_koopman(
    ...
    *,
    ...
    if rollout_steps < 1:
        raise ValueError("Not enough actions for a Koopman rollout.")

    pred_latents = [true_latents[t] for t in range(warmup)]

    # Build initial lifted state from true latents
    z_lifted = true_latents[max(0, warmup - H) : warmup].reshape(-1).unsqueeze(0)  # (1, H*D)
    ...
    for step in range(rollout_steps):
        ...
        act_emb = model.action_encoder(act_raw)[:, 0]  # (1, A)

        z_lifted = model.predict_koopman(z_lifted, act_emb)
...
def plot_latents(
    ...
    *,
    ...
    for plot_idx, (start_dim, end_dim) in enumerate(splits, start=1):
        ...
        fig.suptitle(f"Koopman LE-WM latent rollout episode {episode_idx}, dims {start_dim}-{end_dim - 1}")
        fig.tight_layout()
