...
DEFAULT_DATA_PATH = "obs_height_ogbench/obs_data_height/height_classifier_data.pt"
DEFAULT_OUT_DIR = "obs_height_ogbench/obs_net_height_embd_12_strtn"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--classifier", choices=("mlp", "linear"), default="mlp")
    parser.add_argument(
        "--spectral-norm",
        action="store_true",
        help="Apply spectral normalization to classifier Linear layers.",
    )
...
class ObstacleMLP(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, depth: int, dropout: float, spectral_norm: bool) -> None:
        super().__init__()
        if depth < 1:
        ...
        for _ in range(depth - 1):
            layers.append(make_linear(in_dim, hidden_dim, spectral_norm=spectral_norm))
            layers.append(nn.LayerNorm(hidden_dim))
            layers.append(DEFAULT_ACTIVATION())
            if dropout > 0.0:
                layers.append(nn.Dropout(dropout))
            in_dim = hidden_dim
        layers.append(make_linear(in_dim, 1, spectral_norm=spectral_norm))
...
class ObstacleLinear(nn.Module):
    def __init__(self, input_dim: int, spectral_norm: bool) -> None:
        super().__init__()
        self.linear = make_linear(input_dim, 1, spectral_norm=spectral_norm)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.linear(x).squeeze(-1)


def make_linear(in_dim: int, out_dim: int, *, spectral_norm: bool) -> nn.Module:
...
def build_classifier(
    ...
    *,
    ...
    if classifier == "linear":
        return ObstacleLinear(input_dim=input_dim, spectral_norm=spectral_norm)
    raise ValueError(f"Unknown classifier: {classifier}")


def materialize_spectral_norm_weights(model: nn.Module) -> None:
...
def score_threshold_metrics(eval_payload: dict[str, Any], labels: np.ndarray, threshold: float) -> dict[str, float]:
    ...
    pos_mask = labels < 0.0
    neg_mask = ~pos_mask
    accuracy = float(np.mean(preds_obstacle == pos_mask))
...
def print_accuracy_report(summary: dict[str, Any]) -> None:
    metrics = summary.get("metrics", {})
    if not isinstance(metrics, dict):
        return
    print("Pre-conformal classifier accuracy at score threshold 0.0:")
    for split_name, label in (("train", "Train"), ("val", "Val"), ("cal", "Cal")):
        split_metrics = metrics.get(split_name, {})
        if isinstance(split_metrics, dict) and "accuracy" in split_metrics:
            print(f"{label} acc:  {float(split_metrics['accuracy']):.4f}")


def print_conformal_accuracy_report(summary: dict[str, Any]) -> None:
    ...
    if not isinstance(metrics, dict):
        return
    threshold = conformal.get("safe_score_threshold")
    if threshold is None:
        print("Post-conformal classifier accuracy at safe score threshold:")
    else:
        print(f"Post-conformal classifier accuracy at safe score threshold {float(threshold):.6f}:")
    for split_name, label in (("train", "Train"), ("val", "Val"), ("cal", "Cal")):
        split_metrics = metrics.get(split_name, {})
        if isinstance(split_metrics, dict) and "accuracy" in split_metrics:
            print(f"{label} acc:  {float(split_metrics['accuracy']):.4f}")
...
def main() -> None:
    ...
    if all(path.is_file() for path in paths.required_for_cache_hit) and not args.force_retrain:
        ...
        print_conformal_accuracy_report(summary)
        return
    ...
    if bool(args.spectral_norm):
        materialize_spectral_norm_weights(model)
    ...
    torch.save(
        {
            ...
            "conformal_nonconformity_definition": "max(0, score(x)) on above-threshold calibration samples",
    ...
    torch.save(
        {
            ...
            "conformal": {
                ...
                "delta": float(args.delta),
                "nonconformity_definition": "max(0, score(x)) on above-threshold calibration samples",
    ...
    print_accuracy_report(summary)
    print_conformal_accuracy_report(summary)
    print("Conformal nonconformity: max(0, score(x)) on above-threshold calibration samples")
