"""Obstacle-classifier adapters for the rope latent-safety pipeline."""
...
from rope.safety.obstacle_classifier import ObstacleMLP
...
class RopeObstacleClassifierMargin:
    """Turn the trained rope obstacle classifier into a signed safety margin.
        ...
        *,
        ...
        if self.input_dim != self.latent_dim:
            if not self.allow_latent_slice or self.input_dim > self.latent_dim:
                raise ValueError(
                    "Obstacle classifier latent dimension does not match the rope world-model latent dimension: "
