#!/usr/bin/env python3
"""Build a rope latent cache for the latent-safety/PyHJ pipeline."""
...
from rope.safety.classifier_oracle import RopeObstacleClassifierMargin, transform_margin
from rope.safety.compat import register_legacy_checkpoint_aliases
from rope.safety.constraints import DEFAULT_OBSTACLE_DATA_PATH, compute_rule_margin, load_obstacle_rule
from rope.train.mlpdyn_train import LeWMRopeDataset

DEFAULT_DATASET_PATH = "rope/data/rope_random_cubic_spline_test.h5"
DEFAULT_MODEL_DIR = "rope/models/mlpdyn_noshadow_ft"
DEFAULT_OUTPUT_PATH = "rope/safety/cache/rope_latent_safety_smoke.pt"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--margin-source", choices=("analytic", "classifier"), default="analytic")
...
def make_episode_index(ep_len: np.ndarray, total_frames: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    for ep, length in enumerate(ep_len.tolist()):
        ...
        if cursor >= total_frames:
            break
    if cursor < total_frames:
        raise ValueError("Could not map all selected rows to rope episodes.")
...
def encode_h5_pixels(
    ...
    *,
    ...
    for start in tqdm(range(0, rows.shape[0], frame_batch_size), desc="Encoding rope frames", unit="batch"):
...
def main() -> None:
    ...
    payload = {
        ...
        "task_target": torch.from_numpy(task_target_np).float(),
