#!/usr/bin/env python3
"""Collect a balanced rope obstacle-zone image dataset.

Labels follow the rope obstacle setting, not a global low-rope constraint:
unsafe means the estimated low point of the rope is below the clearance
threshold while the rope reach lies inside the obstacle reach interval.
"""
...
DEFAULT_OUT_DIR = "rope/safety/obstacle_data"
...
LABEL_RULE_NAME = "obstacle_zone_low_rope"
LABEL_RULE_DESCRIPTION = (
    "unsafe iff obstacle_reach_lower <= reach <= obstacle_reach_upper and "
    "estimated_low_rope_height <= low_rope_cutoff"
)
...
def classify_obstacle_states(
    ...
    *,
    ...
    """Classify task states using the obstacle-zone rule.

    This intentionally does not mark every low-rope configuration unsafe. Low
    rope outside the obstacle reach interval is labeled non-obstacle/safe.
    """
