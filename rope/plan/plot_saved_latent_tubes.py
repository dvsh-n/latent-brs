#!/usr/bin/env python3
"""Plot saved SLS latent tubes from plan_sls_mpc_mppi_save_tubes.py."""
...
DEFAULT_ELLIPSOID_PATH = Path("rope/eval/latent_ellipsoid_noshadow/latent_ellipsoid.npz")
FIGURE_GREEN = "#2ca25f"
...
def plot_tubes(args: argparse.Namespace) -> Path:
    ...
    if out_path is None:
        out_path = tube_path.with_name(
            f"latent_tubes_start_{args.start_step:03d}_stride_{args.plan_stride:03d}.png"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--start-step", type=int, default=0, help="First MPC step whose predicted tube can be plotted.")
    ...
    parser.add_argument("--alpha", type=float, default=0.24, help="Initial tube fill alpha.")
