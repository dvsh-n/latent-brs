...
def make_mppi_rollout_and_eval(jax_dynamics_fn, state_dim, action_dim, horizon, W_state, goal_state):
    ...
    def mppi_eval_fn(state_seqs, act_seqs, reach_config=None, aux=None, *args, **kwargs):
        ...
        return {"rewards": -jnp.sum(stage_costs + 0.01 * jnp.sum(act_seqs ** 2, axis=-1), axis=-1)}
...
# def latest_object_checkpoint(model_dir: Path) -> Path:
#     # Captures both styles: _epoch_1_object.ckpt OR _epoch=000.ckpt
...
#             f"No valid checkpoints matching '_epoch_N_object.ckpt' "
#             f"or '_epoch=NNN.ckpt' found in {model_dir}"
#         )
        
    # Return the path associated with the highest epoch integer
    return max(candidates, key=lambda item: item[0])[1]
...
def main():
    ...
    disturbance = make_jax_disturbance(build_equinox_mlp_from_pytorch(error_model.net, key_err), cfg.q_learned, state_dim, error_model.diagonal)
    ...
    mppi_planner = MPPIPlanner(
        ...
        model_rollout_fn=make_mppi_rollout_and_eval(dynamics, state_dim, action_dim, cfg.horizon, W_mppi, jnp.asarray(goal_state))[0],
        evaluate_traj_fn=make_mppi_rollout_and_eval(dynamics, state_dim, action_dim, cfg.horizon, W_mppi, jnp.asarray(goal_state))[1],
    ...
    # SLS Setup footprint
    sls_cfg = SLSConfig(max_sls_iterations=1, sls_primal_tol=1e-2, enable_fastsls=False, initialize_nominal=True, warm_start=True, rti=True)
    controller = GenericMPC(
        sls_cfg, SQPConfig(max_sqp_iterations=1, warm_start=False, feas_tol=1e-2, step_tol=1e-4, line_search=False),
        ADMMConfig(eps_abs=1e-2, eps_rel=1e-4, rho_max=1e2, max_iterations=400, rho_update_frequency=20, initial_rho=1.0),
        config=MPCConfig(n=state_dim, nu=action_dim, N=cfg.horizon, W=W_stage, u_ref=jnp.zeros(action_dim), dt=1.0/30.0),
        dynamics=dynamics, constraints=combine_constraints(make_state_box_constraints(-3.0*jnp.ones(state_dim), 3.0*jnp.ones(state_dim)), make_control_box_constraints(-10.0*jnp.ones(action_dim), 10.0*jnp.ones(action_dim))),
