...
from shared.models_1 import JEPA, MLP, MLPDynamicsPredictor, SIGReg


DEFAULT_DATASET_PATH = "data/expert_data/reacher_expert.h5"
DEFAULT_RUN_DIR = "models/lewm_reacher_mlpdyn_markov_512_2_straighten_more_data"
FIXED_FRAMESKIP = 1
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--straighten-weight", type=float, default=1.0,
                        help="Weight for temporal straightening loss (0 = disabled).")
...
def temporal_straightening_loss(emb: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """Penalise curvature in a latent trajectory via cosine similarity of consecutive velocities."""
    v1 = emb[:, 1:-1] - emb[:, :-2]
    v2 = emb[:, 2:] - emb[:, 1:-1]
...
def lewm_forward(self, batch: dict[str, torch.Tensor], stage: str, args: argparse.Namespace):
    ...
    if args.straighten_weight > 0:
        output["straighten_loss"] = temporal_straightening_loss(emb)
        output["loss"] = output["loss"] + args.straighten_weight * output["straighten_loss"]
