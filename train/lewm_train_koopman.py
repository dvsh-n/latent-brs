#!/usr/bin/env python3
"""Train an LE-WM-style JEPA with a linear Koopman (HVOK) predictor on time-delayed embeddings."""
...
from shared.models import JEPA, MLP, SIGReg, KoopmanPredictor, compute_history_states


DEFAULT_DATASET_PATH = "data/expert_data/reacher_expert.h5"
DEFAULT_RUN_DIR = "models/lewm_reacher_koopman"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-model-name", default="lewm_koopman")
...
def lewm_forward(self, batch: dict[str, torch.Tensor], stage: str, args: argparse.Namespace):
    ...
    emb = output["emb"]          # (B, T, D) where T = history_size + num_preds
    act_emb = output["act_emb"]  # (B, H, A)

    history_states = compute_history_states(emb, H)  # (B, T, H*D)

    input_hist = history_states[:, :H]       # (B, H, H*D)
    target_emb = emb[:, 1 : H + 1]          # (B, H, D) — the z_{t+1} targets
    input_act = act_emb[:, :H]              # (B, H, A)

    B = input_hist.size(0)
    input_hist_flat = input_hist.reshape(B * H, -1)
    input_act_flat = input_act.reshape(B * H, -1)
    target_emb_flat = target_emb.reshape(B * H, -1)

    pred_lifted_flat = self.model.predict_koopman(input_hist_flat, input_act_flat)
    D = emb.size(-1)
    pred_z_flat = pred_lifted_flat[:, -D:]

    output["pred_loss"] = (pred_z_flat - target_emb_flat).pow(2).mean()
