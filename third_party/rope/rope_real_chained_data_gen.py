#!/usr/bin/env python3
"""Collect real-hardware rope data with one smooth chain of task-space splines."""
...
def copy_async_camera_h5(h5: h5py.File, async_h5_path: Path) -> int:
    if not async_h5_path.exists():
        return 0
    with h5py.File(async_h5_path, "r") as src:
        if "async_camera" in h5:
            del h5["async_camera"]
        group = h5.create_group("async_camera")
        for key, value in src.attrs.items():
            group.attrs[key] = value
        for name, dataset in src.items():
            src.copy(dataset, group, name=name)
        return int(src["pixels"].shape[0]) if "pixels" in src else 0
...
def resolve_shard_paths(outdir: Path, output_name: str) -> tuple[int, Path, Path, Path, Path]:
...
def collect_trajectory(
    ...
    *,
    ...
    robot.command_joint_path(q_path, knot_times=sample_times, sample_callback=sample, prepare_start=False)
...
def main() -> None:
    ...
    try:
        ...
        with h5py.File(output_path, "w") as h5:
            ...
            h5.attrs["source"] = "rope/data/rope_real_chained_data_gen.py"
            ...
            h5.attrs["async_camera_full_resolution_frames"] = copy_async_camera_h5(h5, async_h5_path)
    finally:
