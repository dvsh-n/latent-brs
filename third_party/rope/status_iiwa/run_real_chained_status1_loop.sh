...
config="${script_dir}/rope_real_chained_status1_smoke_cl.yaml"
python_bin="/usr/bin/python"
generator="${script_dir}/rope_real_chained_status1_data_gen_cl.py"
...
Runs the status1-only real chained data generator COUNT times.
...
Examples:
  ...
  $0 --count 10 -- --trajectory-mode fixed-point-to-point --fixed-task-start 0.0 1.24 0.36 --fixed-task-goal 0.0 1.24 0.42
EOF
}
...
echo "status1 loop started at $(date)"
echo "count: ${count}"
echo "config: ${config}"
...
while [[ "${run_index}" -lt "${count}" ]]; do
  ...
  echo
  echo "=== status1 run ${run_index}/${count}: seed ${seed} ==="
...
echo
echo "completed status1 runs: ${run_index}"
