...
KOOPMAN_PATH = "models/koop_quad3d_nodec_dagger_1.pt"
DEVICE = "cuda"
...
VIDEO_SAVE_PATH = "plan/videos/plan_admm_mpc_nodec.gif"
...
def print_planning_error_metrics(step_traj_mse, step_pos_traj_mse, k_state_mse_curve, k_pos_mse_curve):
    ...
    print("\n" + "=" * 62)
    print("Planning Model-Error Metrics (Nodec State Block vs True Rollout)")
    print("=" * 62)
...
class GPUADMM_MPCSolver:
    def __init__(self, A_np, B_np, Q_val, R_val, u_min, u_max, horizon, rho=1.0, n_admm_steps=15, device=DEVICE):
        self.H = horizon
    ...
    def _shift_warm_start(self, z0):
        ...
        u_shift = self._u_shift
        z_shift = self._z_shift
        zu_shift = self._zu_shift
        yu_shift = self._yu_shift
...
def plot_3d(x_plan, x_true, start_state, goal_state, planned_trajs=None):
    ...
    ax.plot(x_plan[:, 0], x_plan[:, 1], x_plan[:, 2], "r", linewidth=2, label="Nodec Plan (State Block)")
    ...
    ax.set_ylim(0, 5)
    ax.set_zlim(0, 5)
    ax.set_title("MPC: Nodec State Plan vs True Dynamics")
    ax.legend()
...
def init_live_plot(start_state, goal_state, avg_solve_s):
    ...
    planned_line, = ax.plot([], [], [], "r", linewidth=2, label="Nodec Plan (State Block)")
    true_line, = ax.plot([], [], [], "b", linewidth=2, label="True Dynamics")
    ax.legend()
    plt.tight_layout()
    return fig, planned_line, true_line


def update_live_plot(planned_line, true_line, x_plan, true_traj):
    planned_line.set_data(x_plan[:, 0], x_plan[:, 1])
    planned_line.set_3d_properties(x_plan[:, 2])
...
def show_3d_animation(start_state, goal_state, avg_solve_s, planned_trajs, true_traj, enable_video_gen=False, video_save_path=VIDEO_SAVE_PATH):
    ...
    fig, planned_line, true_line = init_live_plot(start_state, goal_state, avg_solve_s)
...
def main():
    ...
    with torch.inference_mode():
        x0_proc = preprocess_state(start_state, normalizer, enable_normalization)
        xg_proc = preprocess_state(goal_state, normalizer, enable_normalization)
        x0_t = torch.tensor(x0_proc, dtype=torch.float32, device=DEVICE).unsqueeze(0)
        xg_t = torch.tensor(xg_proc, dtype=torch.float32, device=DEVICE).unsqueeze(0)
        z0 = model.lift_state(x0_t).squeeze(0)
        zg = model.lift_state(xg_t).squeeze(0)
    ...
    for _ in pbar:
        ...
        with torch.inference_mode():
            x_proc = preprocess_state(current_state, normalizer, enable_normalization)
            x_t = torch.tensor(x_proc, dtype=torch.float32, device=DEVICE).unsqueeze(0)
            z_curr = model.lift_state(x_t).squeeze(0)
        ...
        with torch.inference_mode():
            x_plan_proc = z_traj_t[:, :model.state_dim].cpu().numpy()
            x_plan = postprocess_state(x_plan_proc, normalizer, enable_normalization)
        ...
        sq_err = (x_plan[1:] - x_true_rollout[1:]) ** 2
