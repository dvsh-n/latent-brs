...
KOOPMAN_PATH = "models/koop_quad3d_lindec_dagger_1.pt"
DEVICE = "cuda"
...
VIDEO_SAVE_PATH = "plan/videos/plan_admm_mpc_lindec.gif"
...
def print_planning_error_metrics(step_traj_mse, step_pos_traj_mse, k_state_mse_curve, k_pos_mse_curve):
    ...
    print("\n" + "=" * 62)
    print("Planning Model-Error Metrics (Linear Decoder Cz vs True Rollout)")
    print("=" * 62)
...
def plot_3d(x_plan, x_true, start_state, goal_state, planned_trajs=None):
    ...
    ax.plot(x_plan[:, 0], x_plan[:, 1], x_plan[:, 2], "r", linewidth=2, label="LinDec Plan (Cz)")
    ...
    ax.set_ylim(0, 5)
    ax.set_zlim(0, 5)
    ax.set_title("MPC: LinDec Cz Plan vs True Dynamics")
    ax.legend()
...
def init_live_plot(start_state, goal_state, avg_solve_s):
    ...
    planned_line, = ax.plot([], [], [], "r", linewidth=2, label="LinDec Plan (Cz)")
...
def main():
    ...
    for _ in pbar:
        ...
        with torch.inference_mode():
            x_plan_proc = model.C(z_traj_t).cpu().numpy()
