...
MODEL_SAVE_PATH = "models/koop_quad3d_nodec_dagger.pt"
PLOT_SAVE_DIR = "eval/eval_plots_nodec"
N_EVAL_EPISODES = 25
MAX_TIMESTEP = 100
...
def parse_eval_args():
    parser = argparse.ArgumentParser(description="Evaluate decoder-free Deep Koopman model")
...
def rollout_latent(model, x0_raw, u_raw, dataset, device, enable_normalization):
    ...
    with torch.no_grad():
        ...
        z_k = model.lift_state(x0_proc)

        x_hat_proc_list = [x0_proc.squeeze(0)]
        for k in range(H):
            u_k = u_raw[k].unsqueeze(0)
            z_k = model.A(z_k) + model.B(u_k)
            x_hat_proc_list.append(z_k[:, :model.state_dim].squeeze(0))
...
def rollout_recurrent(model, x0_raw, u_raw, dataset, device, enable_normalization):
    ...
    with torch.no_grad():
        ...
        for k in range(H):
            ...
            z_k = model.lift_state(x_k_proc)
            u_k = u_raw[k].unsqueeze(0)
            z_kp1 = model.A(z_k) + model.B(u_k)
            x_kp1_proc = z_kp1[:, :model.state_dim]
...
def plot_rollout(x_traj: np.ndarray, x_hat_traj: np.ndarray, episode_idx: int, plot_save_dir: str):
    ...
    fig.suptitle(f"Decoder-Free Koopman Rollout vs. Ground Truth (Episode {episode_idx})", fontsize=20)
...
def evaluate(model_save_path_arg=None, data_path_arg=None, plot_save_dir_arg=None,
    ...
    try:
        ...
        print("Please run 'koop_train_nodec.py' first.")
        return
    ...
    print_rmse_table(grouped_labels, grouped_rmse, title="Decoder-Free Koopman 3D Performance Metrics (RMSE)")
