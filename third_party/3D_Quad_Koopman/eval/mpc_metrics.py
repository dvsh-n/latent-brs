...
MODEL_PATH = "models/koop_quad3d_dagger.pt"
DEVICE = "cuda"
...
def evaluate_horizon(model, normalizer, solver, starts, goals, device, max_mpc_steps, goal_tol):
    n = starts.shape[0]
    curr = starts.copy()
    ...
    with torch.inference_mode():
        goals_norm = normalizer.normalize(goals)
        goals_t = torch.tensor(goals_norm, dtype=torch.float32, device=device)
        z_goals = model.encoder(goals_t)
    ...
    for step in range(max_mpc_steps):
        ...
        with torch.inference_mode():
            curr_norm = normalizer.normalize(curr[active_idx])
            curr_t = torch.tensor(curr_norm, dtype=torch.float32, device=device)
            z_curr = model.encoder(curr_t)
            ...
            bsz = z_plan.shape[0]
            x_dec_norm = model.decoder(z_plan.reshape(-1, z_plan.shape[-1]))
            x_dec = normalizer.denormalize(x_dec_norm.cpu().numpy()).reshape(bsz, solver.H + 1, 12)
        ...
        for j, idx in enumerate(active_idx):
            ...
            sq_err = (x_dec[j, 1:] - x_true_rollout[1:]) ** 2
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate ADMM-MPC metrics across horizon sweep")
