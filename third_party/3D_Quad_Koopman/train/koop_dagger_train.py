"""
DAgger training for Deep Koopman quadrotor with GPU batch ADMM planning.

Differences from test/dagger/dagger_loop.py:
1) Replaces CasADi planner with GPU ADMM batch trajectory optimization.
2) Stores DAgger data/checkpoints under train/dagger/.
3) Exports final model to models/koop_quad3d_dagger.pt.
"""
import json
import os
import shutil
...
from train.koop_train import QuadrotorDataset
...
# ====== PATHS ======
TRAIN_DAGGER_DIR = ROOT_DIR / "train" / "dagger"
DAGGER_DATA_PATH = "train/dagger/data/dagger_data.pt"
DAGGER_CKPT_DIR = "train/dagger/ckpts"
DAGGER_LOG_DIR = "train/dagger/runs"
DAGGER_EVAL_DIR = "train/dagger/eval"
DAGGER_EVAL_LOG_DIR = "train/dagger/eval_runs"
FINAL_MODEL_PATH = "models/koop_quad3d_dagger.pt"
...
def rollout_model(model, normalizer, x0, u_seq):
    device = next(model.parameters()).device
    with torch.no_grad():
        x0_norm = normalizer.normalize(x0)
        ...
        z_traj_t = torch.cat(z_traj, dim=0)
        x_hat_norm = model.decoder(z_traj_t)
        x_hat = normalizer.denormalize(x_hat_norm.cpu().numpy())
    return x_hat
...
def evaluate_iteration(model, normalizer, iteration, writer=None):
    ...
    with torch.no_grad():
        ...
        z0 = model.encoder(x0_t).cpu().numpy()
        zg = model.encoder(xg_t).cpu().numpy()
    ...
    for ep in range(EVAL_EPISODES):
        ...
        x_pred = rollout_model(model, normalizer, starts[ep], u_traj)
...
def train_iteration(data_path, ckpt_path, log_dir, iteration, load_from_ckpt=None):
    ...
    dataset = QuadrotorDataset(data_path, MULTI_STEP_HORIZON)
    ...
    for epoch in tqdm(range(EPOCHS_PER_ITER), desc=f"Train iter {iteration}"):
        for x_k, u_seq, x_kp1_seq in dataloader:
            ...
            x_k_hat, x_target_seq_hat, z_pred_seq, x_pred_seq_hat, z_target_seq = model(x_k, u_seq, x_kp1_seq)
            ...
            loss = LAMBDA_RECON * loss_autoencoder + LAMBDA_LINEAR * loss_dynamics
...
def collect_and_aggregate(model, normalizer, rng, data_path, n_traj):
    device = next(model.parameters()).device
    A_np = model.A.weight.detach().cpu().numpy()
    B_np = model.B.weight.detach().cpu().numpy()
    solver = GPUADMMBatchTrajOptSolver(
    ...
    with torch.no_grad():
        ...
        for i in range(0, n_traj, ENCODE_BATCH):
            ...
            x0_t = torch.tensor(x0_norm[i:j], dtype=torch.float32, device=device)
            xg_t = torch.tensor(xg_norm[i:j], dtype=torch.float32, device=device)
...
def cleanup_dagger_workspace():
    """Delete train/dagger workspace used during DAgger runs."""
...
def main():
    ...
    # Always start from a fresh initial dataset; do not reuse prior aggregated DAgger data.
    init_dagger_data()
    ...
    for it in range(NUM_ITERATIONS):
        ...
        evaluate_iteration(model, normalizer, it, eval_writer)
        collect_and_aggregate(model, normalizer, rng, data_path, NEW_TRAJ_PER_ITER)
