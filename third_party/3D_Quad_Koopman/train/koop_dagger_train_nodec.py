"""
DAgger training for decoder-free Deep Koopman quadrotor with GPU batch ADMM planning.
"""
import json
import os
import shutil
...
from train.koop_train_nodec import QuadrotorDataset
...
TRAIN_DAGGER_DIR = ROOT_DIR / "train" / "dagger_nodec"
DAGGER_DATA_PATH = "train/dagger_nodec/data/dagger_data.pt"
DAGGER_CKPT_DIR = "train/dagger_nodec/ckpts"
DAGGER_LOG_DIR = "train/dagger_nodec/runs"
DAGGER_EVAL_DIR = "train/dagger_nodec/eval"
DAGGER_EVAL_LOG_DIR = "train/dagger_nodec/eval_runs"
FINAL_MODEL_PATH = "models/koop_quad3d_nodec_dagger.pt"
...
def rollout_model(model, normalizer, x0, u_seq, enable_normalization):
    ...
    with torch.no_grad():
        ...
        z_k = model.lift_state(x0_t)

        x_hat_proc = [x0_t.squeeze(0)]
        for u_k in u_seq:
            u_t = torch.tensor(u_k, dtype=torch.float32, device=device).unsqueeze(0)
            z_k = model.A(z_k) + model.B(u_t)
            x_hat_proc.append(z_k[:, : model.state_dim].squeeze(0))
...
def evaluate_iteration(model, normalizer, iteration, enable_normalization, writer=None):
    ...
    with torch.no_grad():
        ...
        for i in range(0, EVAL_EPISODES, ENCODE_BATCH):
            ...
            z0_parts.append(model.lift_state(x0_t).cpu().numpy())
            zg_parts.append(model.lift_state(xg_t).cpu().numpy())
...
def train_iteration(data_path, ckpt_path, log_dir, iteration, load_from_ckpt=None):
    ...
    model = DeepKoopmanNoDec(**model_config).to(device)
