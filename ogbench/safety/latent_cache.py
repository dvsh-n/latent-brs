#!/usr/bin/env python3
"""Build an OGBench cube latent cache for the latent-safety/PyHJ pipeline."""
...
from ogbench.safety.classifier_oracle import OGBenchCubeObstacleClassifierMargin, transform_margin
...
from ogbench.train.lewm_train_mlp_markov import LeWMOGBenchDataset

DEFAULT_DATASET_PATH = "ogbench/data/expert_data/ogbench_cube_expert.h5"
DEFAULT_MODEL_DIR = "ogbench/models/mlpdyn"
DEFAULT_OUTPUT_PATH = "ogbench/safety/cache/cube_latent_safety_smoke.pt"
...
def load_action_stats(config: dict[str, object], dataset_path: Path) -> tuple[np.ndarray, np.ndarray]:
    stats_dataset = LeWMOGBenchDataset(
...
def make_episode_index(ep_len: np.ndarray, total_frames: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    for ep, length in enumerate(ep_len.tolist()):
        ...
        if cursor >= total_frames:
            break
    if cursor < total_frames:
        raise ValueError("Could not map all selected rows to ogbench episodes.")
...
def encode_h5_pixels(
    ...
    *,
    ...
    for start in tqdm(range(0, rows.shape[0], frame_batch_size), desc="Encoding ogbench frames", unit="batch"):
...
def compute_margins(
    ...
    *,
    ...
    if args.margin_source == "analytic":
        ...
        raw_margins = torch.from_numpy(compute_rule_margin(block_pos, rule)).float()
    ...
    classifier = OGBenchCubeObstacleClassifierMargin(
...
def main() -> None:
    ...
    payload = {
        ...
        "block_pos": torch.from_numpy(block_pos_np).float(),
