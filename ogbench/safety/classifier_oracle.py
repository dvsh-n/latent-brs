"""Obstacle-classifier adapters for the ogbench latent-safety pipeline."""
...
from ogbench.safety.obstacle_classifier import ObstacleMLP
...
class OGBenchCubeObstacleClassifierMargin:
    """Turn the trained ogbench obstacle classifier into a signed safety margin.
        ...
        *,
        ...
        if self.input_dim != self.latent_dim:
            if not self.allow_latent_slice or self.input_dim > self.latent_dim:
                raise ValueError(
                    "Obstacle classifier latent dimension does not match the ogbench world-model latent dimension: "
