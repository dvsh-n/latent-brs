#!/usr/bin/env python3
"""Train an LE-WM-style JEPA with a Markov latent-state MLP dynamics predictor on OGBench."""
...
DEFAULT_DATASET_PATH = "ogbench/data/cube_single_expert.h5"
DEFAULT_RUN_DIR = "ogbench/models/lewm_ogbench_mlpdyn_markov"
FINETUNE_DIR = None
DEFAULT_FRAMESKIP = 1
...
class LeWMOGBenchDataset(Dataset):
    def __init__(
        self,
        dataset_path: Path,
        *,
        ...
        with h5py.File(self.dataset_path, "r") as h5:
            ...
            if finite_actions.size == 0:
                raise ValueError("No finite actions found in OGBench dataset.")
        ...
        if not self.samples:
            raise ValueError("No valid OGBench training windows found. Check frameskip/history/num_preds.")
...
def main() -> None:
    ...
    if not 0.0 < args.train_split <= 1.0:
        raise ValueError("--train-split must be greater than 0 and at most 1.")

    dataset = LeWMOGBenchDataset(
        dataset_path,
