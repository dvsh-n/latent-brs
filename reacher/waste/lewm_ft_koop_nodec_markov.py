#!/usr/bin/env python3
"""Finetune a decoder-free Koopman Markov LE-WM model from a saved object checkpoint."""
...
DEFAULT_INIT_MODEL_DIR = "reacher/models/lewm_reacher_koopdyn_markov_1"
DEFAULT_RUN_DIR = "reacher/models/lewm_reacher_koopdyn_markov_ft_15"
FIXED_FRAMESKIP = 1


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--init-model-dir", type=Path, default=DEFAULT_INIT_MODEL_DIR)
    ...
    parser.add_argument("--num-preds", type=int, default=15, help="Linear Koopman rollout horizon.")
    ...
    if args.init_model_dir is None and args.init_checkpoint is None:
        parser.error("Pass either --init-model-dir or --init-checkpoint for finetuning.")
    if args.init_model_dir is not None and args.init_checkpoint is not None:
        parser.error("Pass only one of --init-model-dir or --init-checkpoint.")
...
def resolve_init_checkpoint(args: argparse.Namespace) -> Path:
    if args.init_checkpoint is not None:
        ...
        checkpoint_path = latest_object_checkpoint(args.init_model_dir.expanduser().resolve()).resolve()
    if not checkpoint_path.is_file():
        raise FileNotFoundError(f"Finetune checkpoint not found: {checkpoint_path}")
    return checkpoint_path


def load_finetune_model(checkpoint_path: Path, args: argparse.Namespace) -> JEPA:
    finetune_model = torch.load(checkpoint_path, map_location="cpu", weights_only=False)
    if not isinstance(finetune_model, JEPA):
        raise TypeError(f"Expected JEPA object checkpoint, got {type(finetune_model).__name__}.")
    predictor = getattr(finetune_model, "predictor", None)
    if not isinstance(predictor, KoopmanDynamicsPredictor):
        raise TypeError(
            "Finetune checkpoint does not contain a decoder-free Koopman predictor. "
            f"Got {type(predictor).__name__ if predictor is not None else 'None'}."
        )

    expected_markov_state_dim = 2 * args.embed_dim
    expected_action_dim = args.frameskip * args.action_dim
    if int(getattr(predictor, "state_dim", -1)) != expected_markov_state_dim:
        raise ValueError(
            f"Checkpoint state_dim={getattr(predictor, 'state_dim', None)} does not match "
            f"requested markov_state_dim={expected_markov_state_dim}."
        )
    if int(getattr(predictor, "action_dim", -1)) != expected_action_dim:
        raise ValueError(
            f"Checkpoint action_dim={getattr(predictor, 'action_dim', None)} does not match "
            f"requested action_dim={expected_action_dim}."
        )
    if int(getattr(predictor, "koopman_embed_dim", -1)) != args.koop_embedding_dim:
        raise ValueError(
            f"Checkpoint koopman_embed_dim={getattr(predictor, 'koopman_embed_dim', None)} does not match "
            f"requested koop_embedding_dim={args.koop_embedding_dim}."
        )
...
def main() -> None:
    ...
    finetune_model = load_finetune_model(init_checkpoint_path, args)
    optimizers = {
        "model_opt": {
            "modules": "model",
