...
def build_mlp(
    ...
    layers: list[nn.Module] = [nn.Linear(input_dim, hidden_width), activation_layer()]
    for _ in range(depth - 1):
        layers.extend([nn.Linear(hidden_width, hidden_width), activation_layer()])
    layers.append(nn.Linear(hidden_width, output_dim))
    return nn.Sequential(*layers)


class DinoPatchCnnAdapter(nn.Module):
    """Map a grid of DINO patch tokens to a compact state representation."""
    ...
    def __init__(
        ...
        *,
        ...
        if grid_size * grid_size != num_patch_tokens:
            raise ValueError(f"num_patch_tokens={num_patch_tokens} does not form a square grid")
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(self.token_dim, hidden_channels, kernel_size=1),
            nn.GELU(),
        ...
        self.head = nn.Linear(hidden_channels, self.state_dim)

    def forward(self, patch_tokens: torch.Tensor) -> torch.Tensor:
        if patch_tokens.ndim not in (3, 4):
            raise ValueError(
                f"Expected patch token tensor with 3 or 4 dims, got shape {tuple(patch_tokens.shape)}"
            )

        leading_shape = patch_tokens.shape[:-2]
        n_tokens, token_dim = patch_tokens.shape[-2:]
        if n_tokens != self.num_patch_tokens:
            raise ValueError(f"Expected {self.num_patch_tokens} patch tokens, got {n_tokens}")
        if token_dim != self.token_dim:
            raise ValueError(f"Expected token dim {self.token_dim}, got {token_dim}")

        x = patch_tokens.reshape(-1, n_tokens, token_dim)
        x = self.token_norm(x)
        x = x.transpose(1, 2).contiguous()
        x = x.view(-1, self.token_dim, self.grid_size, self.grid_size)
        x = self.conv(x)
        x = self.pool(x).flatten(1)
        x = self.head(x)
        return x.view(*leading_shape, self.state_dim)


class HistoryMLPObservableEncoder(nn.Module):
    """Encode a sliding window of compact states into lifted observables."""
    ...
    def __init__(
        ...
        *,
        ...
        self.net = build_mlp(
            self.window_size * self.state_dim,
            self.observable_dim,
            hidden_width,
            depth,
            activation_layer,
    ...
    def forward(self, state_windows: torch.Tensor) -> torch.Tensor:
        if state_windows.ndim != 4:
            raise ValueError(
                f"Expected state windows with shape [B, W, T, D], got {tuple(state_windows.shape)}"
            )
        if state_windows.shape[-2] != self.window_size:
            raise ValueError(f"Expected history window size {self.window_size}, got {state_windows.shape[-2]}")
        if state_windows.shape[-1] != self.state_dim:
            raise ValueError(f"Expected state dim {self.state_dim}, got {state_windows.shape[-1]}")

        batch_size, n_windows = state_windows.shape[:2]
        flat = state_windows.reshape(batch_size * n_windows, self.window_size * self.state_dim)
        observables = self.net(flat)
        return observables.view(batch_size, n_windows, self.observable_dim)


class VisualHistoryKoopman(nn.Module):
    """
    Learn compact visual states x_t from DINO patch tokens and lift them with
    a history-conditioned observable encoder before linear Koopman rollout.
    """
    ...
    def __init__(
        ...
        *,
        ...
        observable_hidden_width: int,
        observable_hidden_depth: int,
        ...
        self.latent_dim = self.state_dim + self.observable_dim

        self.state_adapter = DinoPatchCnnAdapter(
            token_dim=self.token_dim,
            num_patch_tokens=self.num_patch_tokens,
            state_dim=self.state_dim,
            hidden_channels=adapter_hidden_channels,
        )
        self.observable_encoder = HistoryMLPObservableEncoder(
            state_dim=self.state_dim,
            history_len=self.history_len,
            observable_dim=self.observable_dim,
            hidden_width=observable_hidden_width,
            depth=observable_hidden_depth,
    ...
    def _select_patch_tokens(self, latent_seq: torch.Tensor) -> torch.Tensor:
        if latent_seq.ndim != 4:
            raise ValueError(
                f"Expected latent sequence with shape [B, T, N, D], got {tuple(latent_seq.shape)}"
            )

        n_tokens = latent_seq.shape[-2]
        token_dim = latent_seq.shape[-1]
        if token_dim != self.token_dim:
            raise ValueError(f"Expected token dim {self.token_dim}, got {token_dim}")
        ...
        if self.has_cls_token:
            ...
            if n_tokens != expected:
                raise ValueError(f"Expected {expected} tokens including CLS, got {n_tokens}")
            return latent_seq[..., 1:, :]
    ...
    def encode_states(self, latent_seq: torch.Tensor) -> torch.Tensor:
        patch_tokens = self._select_patch_tokens(latent_seq)
        return self.state_adapter(patch_tokens)

    def build_state_windows(self, state_seq: torch.Tensor) -> torch.Tensor:
        seq_len = state_seq.shape[1]
        if seq_len < self.window_size:
            raise ValueError(f"Need at least {self.window_size} states, got {seq_len}")
        n_windows = seq_len - self.history_len
        return torch.stack(
            [state_seq[:, start : start + self.window_size, :] for start in range(n_windows)],
            dim=1,
        )

    def encode_observables(self, state_seq: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        state_windows = self.build_state_windows(state_seq)
        observables = self.observable_encoder(state_windows)
        current_states = state_seq[:, self.history_len :, :]
        return current_states, observables

    def lift_latent_sequence(self, latent_seq: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        state_seq = self.encode_states(latent_seq)
        current_states, observables = self.encode_observables(state_seq)
        lifted_seq = torch.cat([current_states, observables], dim=-1)
        return lifted_seq, current_states

    def lift_history(self, latent_history_seq: torch.Tensor) -> torch.Tensor:
        if latent_history_seq.shape[1] != self.window_size:
            raise ValueError(
                f"Expected exactly {self.window_size} latent frames, got {latent_history_seq.shape[1]}"
            )
        lifted_seq, _ = self.lift_latent_sequence(latent_history_seq)
        return lifted_seq[:, -1, :]

    def forward(self, latent_seq: torch.Tensor, u_seq: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        if u_seq.ndim != 3:
            raise ValueError(f"Expected control sequence [B, M, U], got {tuple(u_seq.shape)}")
        if u_seq.shape[-1] != self.control_dim:
            raise ValueError(f"Expected control dim {self.control_dim}, got {u_seq.shape[-1]}")

        lifted_seq, current_states = self.lift_latent_sequence(latent_seq)
        n_targets = u_seq.shape[1]
        if lifted_seq.shape[1] != n_targets + 1:
            raise ValueError(
                "Latent sequence length and history length do not match the control rollout horizon: "
                f"got lifted windows={lifted_seq.shape[1]} and controls={n_targets}"
            )

        z_k = lifted_seq[:, 0, :]
        z_target_seq = lifted_seq[:, 1:, :]
        x_target_seq = current_states[:, 1:, :]

        z_pred_list = []
        z_current = z_k
        for step in range(n_targets):
            u_k = u_seq[:, step, :]
            z_current = self.A(z_current) + self.B(u_k)
            z_pred_list.append(z_current)

        z_pred_seq = torch.stack(z_pred_list, dim=1)
        x_pred_seq = z_pred_seq[..., : self.state_dim]
        return z_pred_seq, x_pred_seq, z_target_seq, x_target_seq
