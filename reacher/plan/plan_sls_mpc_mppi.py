#!/usr/bin/env python3
"""Plan in Reacher pixel space using MPPI-warmstarted conformal SLS MPC over a world model."""
...
class PlanSLSReacherConfig:
    """Configuration for MPPI-warmstarted conformal SLS MPC Reacher planning."""
    ...
    out_dir: Path = field(default=Path("reacher/plan/sls_mppi_conformal"))
...
def main():
    ...
    pbar = tqdm(range(cfg.max_mpc_steps), desc="Reacher MPPI + conformal SLS execution loop")
    ...
    print(f"Reacher MPPI + SLS MPC planning sequence logged inside: {run_dir}")
