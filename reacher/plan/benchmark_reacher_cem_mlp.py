#!/usr/bin/env python3
"""Hard Reacher benchmark using CEM over our MLP latent dynamics model.

This is a planner-ablation companion to ``benchmark_reacher_hard.py``:
it uses the same learned Reacher MLP dynamics checkpoint and hard endpoint
protocol, but replaces iLQR with CEM for MPC action selection.
"""
...
DEFAULT_OUT_DIR = "reacher/plan/reacher_cem_mlp_eval"
DEFAULT_NUM_EVAL = 50
DEFAULT_SEED = 42
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-dir", type=Path, default=Path(ilqr_base.DEFAULT_MODEL_DIR))
    parser.add_argument("--checkpoint", type=Path, default=None)
    parser.add_argument("--dataset-path", type=Path, default=Path(ilqr_base.DEFAULT_TEST_DATASET_PATH))
...
class CEMMPCSolver:
    ...
    def _initial_distrib(self, init_action: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        mean = torch.zeros((0, self.action_dim), dtype=torch.float32, device=self.device) if init_action is None else init_action.to(self.device)
        ...
        remaining = self.horizon - mean.shape[0]
        if remaining > 0:
            mean = torch.cat(
                (mean, torch.zeros((remaining, self.action_dim), dtype=torch.float32, device=self.device)),
                dim=0,
            )
        elif remaining < 0:
    ...
    def _cost(self, x0: torch.Tensor, x_goal: torch.Tensor, actions: torch.Tensor) -> torch.Tensor:
        ...
        for step in range(self.horizon):
            state_err = x - x_goal.unsqueeze(0)
            costs = costs + self.q_stage * torch.sum(state_err * state_err, dim=-1)
        ...
        costs = costs + self.q_terminal * torch.sum(terminal_err * terminal_err, dim=-1)
        return costs
    ...
    def solve(
        ...
        mean, std = self._initial_distrib(init_action)

        ilqr_base.maybe_cuda_synchronize(self.device)
        ...
        for _ in range(self.num_iters):
            ...
            final_cost = float(elite_costs.mean().item())

        ilqr_base.maybe_cuda_synchronize(self.device)
        solve_time = time.perf_counter() - t0
        return mean.detach().cpu().numpy().astype(np.float32), solve_time, final_cost
...
class CEMPolicyAdapter:
    ...
    def reset(self, *, goal_embedding: torch.Tensor) -> None:
        ...
        self.next_init = None
        self.goal_state = ilqr_base.make_markov_state(goal_embedding).detach().cpu().numpy().astype(np.float32)

    def get_action(self, current_embedding: torch.Tensor) -> tuple[np.ndarray, np.ndarray, dict[str, float]]:
        if self.goal_state is None:
            raise RuntimeError("CEMPolicyAdapter.reset() must be called before get_action().")
        ...
        if len(self.action_buffer) == 0:
            ...
            u_plan, solve_time, plan_cost = self.solver.solve(current_state_np, self.goal_state, init_action=self.next_init)
            keep = min(self.receding_horizon, u_plan.shape[0])
            rest = u_plan[keep:]
            self.next_init = torch.tensor(rest, dtype=torch.float32) if self.warm_start else None
        ...
        u0_norm = self.action_buffer.popleft().astype(np.float32)
        ...
        return u0_raw, u0_norm, record


def load_assets(args: argparse.Namespace, device: torch.device) -> tuple[CEMPolicyAdapter, torch.nn.Module, dict[str, Any], torch.Tensor, torch.Tensor, Path]:
    ...
    if history_size != 1:
        raise ValueError(f"Expected history_size=1, got {history_size}.")

    stats_dataset_path = (
        args.stats_dataset_path.expanduser().resolve()
        if args.stats_dataset_path is not None
        else args.dataset_path.expanduser().resolve()
    )
    stats_dataset = ilqr_base.LeWMReacherDataset(
        stats_dataset_path,
    ...
    method_config = {
        ...
        "warm_start": not bool(args.no_warm_start),
        "matches_stable_worldmodel_cem_defaults": (
            int(args.horizon) == 5
            and int(args.receding_horizon) == 5
            and int(args.action_block) == 1
            and int(args.cem_samples) == 300
            and int(args.cem_iters) == 30
            and int(args.cem_topk) == 30
            and float(args.cem_var_scale) == 1.0
        ),
        "action_dim": action_dim,
        "img_size": img_size,
    }
    return policy, model, method_config, stats_dataset.pixel_mean, stats_dataset.pixel_std, checkpoint_path
...
def run_case(
    *,
    ...
    out_root: Path,
    device: torch.device,
    assets: tuple[CEMPolicyAdapter, torch.nn.Module, dict[str, Any], torch.Tensor, torch.Tensor, Path],
) -> dict[str, Any]:
    policy, model, model_config, pixel_mean, pixel_std, _ = assets
    ...
    goal_emb = ilqr_base.encode_single_frame(
        model,
        goal_frame,
        device=device,
        img_size=img_size,
    ...
    policy.reset(goal_embedding=goal_emb)
    ...
    for step in range(int(args.eval_budget)):
        ...
        action_raw, action_norm, record = policy.get_action(current_emb)
...
def build_metrics(
    *,
    ...
    return {
        ...
        "episode_strict_observation_successes": strict_successes.astype(int).tolist(),
...
def main() -> None:
    ...
    if args.receding_horizon < 1:
        raise ValueError("--receding-horizon must be positive.")
    if args.receding_horizon > args.horizon:
        raise ValueError("--receding-horizon must be <= --horizon.")
    if args.action_block != 1:
        raise ValueError("The Reacher MLP dynamics CEM ablation currently supports only --action-block 1.")
    if args.cem_iters < 1:
    ...
    run_name = f"{int(time.time())}_cem_mlp_seed_{args.seed}"
    out_root = args.out_dir.expanduser().resolve() / run_name
    out_root.mkdir(parents=True, exist_ok=True)

    case_results = []
    for case_idx, case in enumerate(tqdm(cases, desc="Reacher CEM-MLP eval")):
        case_results.append(
            run_case(
                args=args,
                case=case,
