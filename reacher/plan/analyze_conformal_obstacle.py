#!/usr/bin/env python3
"""Analyze a local conformalized latent obstacle for the Reacher planner."""
...
def zonotope_coords(points: np.ndarray, zonotope: dict[str, np.ndarray | float]) -> np.ndarray:
    center = np.asarray(zonotope["center"], dtype=np.float64)
    right_pinv = np.asarray(zonotope["right_pinv"], dtype=np.float64)
...
def build_conformal_zonotope(
    ...
    *,
    ...
    g_hat = eigvecs @ np.diag(sqrt_eigs)
    right_pinv = eigvecs @ np.diag(inv_sqrt_eigs)

    def excess(points: np.ndarray) -> np.ndarray:
        coords = (points - center) @ right_pinv
        return np.maximum(0.0, np.abs(coords) - 1.0)

    norm_excess = excess(norm_samples)
    vmax = norm_excess.max(axis=0)
    lambdas = 1.0 / (vmax + eps)

    cal_excess = excess(cal_samples)
    scores = np.max(lambdas[None, :] * cal_excess, axis=1)
    conformal_radius = conformal_quantile(scores, delta=delta)

    inflation = 1.0 + conformal_radius / lambdas
    g_bar = g_hat @ np.diag(inflation)
    right_pinv_bar = eigvecs @ np.diag(1.0 / (sqrt_eigs * inflation))
    ...
    return {
        ...
        "base_generators": g_hat.astype(np.float64),
        "generators": g_bar.astype(np.float64),
        "lambdas": lambdas.astype(np.float64),
        "inflation": inflation.astype(np.float64),
        "conformal_radius": float(conformal_radius),
        "right_pinv": right_pinv_bar.astype(np.float64),
    }


def zonotope_contains(points: np.ndarray, zonotope: dict[str, np.ndarray | float], tol: float) -> np.ndarray:
    center = np.asarray(zonotope["center"], dtype=np.float64)
    right_pinv = np.asarray(zonotope["right_pinv"], dtype=np.float64)
    coords = (points - center) @ right_pinv
    return np.max(np.abs(coords), axis=1) <= (1.0 + tol)


def project_zonotope(
    zonotope: dict[str, np.ndarray | float],
    pca: dict[str, np.ndarray],
) -> tuple[np.ndarray, np.ndarray]:
    center = np.asarray(zonotope["center"], dtype=np.float64)
    generators = np.asarray(zonotope["generators"], dtype=np.float64)
    center_2d = (center - pca["mean"]) @ pca["components"].T
    generators_2d = (pca["components"] @ generators).T
    return center_2d, generators_2d


def zonotope_boundary_2d(center: np.ndarray, generators: np.ndarray, tol: float = 1e-12) -> np.ndarray:
    if generators.ndim != 2 or generators.shape[1] != 2:
        raise ValueError(f"Expected generators with shape (m, 2), got {generators.shape}.")
    active = generators[np.linalg.norm(generators, axis=1) > tol]
    if active.shape[0] == 0:
        return center.reshape(1, 2)
    angles = np.mod(np.arctan2(active[:, 1], active[:, 0]), np.pi)
    order = np.argsort(angles)
    active = active[order]
    vertices = []
    current = center - active.sum(axis=0)
    vertices.append(current.copy())
    for generator in active:
        current = current + 2.0 * generator
        vertices.append(current.copy())
    for generator in active:
        current = current - 2.0 * generator
        vertices.append(current.copy())
...
def make_plot(
    ...
    *,
    ...
    if zonotope_boundary.shape[0] >= 2:
        ax.plot(zonotope_boundary[:, 0], zonotope_boundary[:, 1], color="tab:orange", linewidth=2.0, label="zonotope boundary", zorder=10)
    ...
    if focus_main_view:
        ...
        if zonotope_boundary.shape[0] >= 2:
            focus_items.append(zonotope_boundary)
    ...
    if add_zoom_inset:
        ...
        if zonotope_boundary.shape[0] >= 2:
            zoom_items.append(zonotope_boundary)
        ...
        if zonotope_boundary.shape[0] >= 2:
            zoom_ax.plot(zonotope_boundary[:, 0], zonotope_boundary[:, 1], color="tab:orange", linewidth=2.0, zorder=10)
        zoom_ax.scatter(
            nominal_point[0],
            nominal_point[1],
            s=80,
...
def make_zonotope_coords_plot(
    ...
    *,
    ...
    ax.axvspan(-1.0, 1.0, ymin=0.0, ymax=1.0, color="tab:orange", alpha=0.08, zorder=0)
    ax.axhspan(-1.0, 1.0, xmin=0.0, xmax=1.0, color="tab:orange", alpha=0.08, zorder=0)
    square = np.array(
        [
            [-1.0, -1.0],
            [1.0, -1.0],
    ...
    ax.plot(square[:, 0], square[:, 1], color="tab:orange", linewidth=2.0, label="unit box in zonotope coords", zorder=5)
    ...
    ax.set_xlabel("Zonotope coord 1")
    ax.set_ylabel("Zonotope coord 2")
    ax.set_title("Coverage view in zonotope coordinates")
    ax.grid(True, alpha=0.25)
    ax.legend(loc="best")
...
def main() -> None:
    ...
    zonotope = build_conformal_zonotope(
        pca_samples,
        norm_samples,
        cal_samples,
        delta=args.delta,
    ...
    obstacle_coords = zonotope_coords(all_obstacle_samples, zonotope)
    construction_coords = obstacle_coords[:cal_end]
    test_coords = obstacle_coords[cal_end:]
    make_zonotope_coords_plot(
        ...
        nominal_coords=zonotope_coords(nominal_position[None, :], zonotope)[0, :2],
    )
    torch.save(
        {
    ...
    save_json(
        episode_dir / "summary.json",
        {
            ...
            "conformal_radius": float(zonotope["conformal_radius"]),
