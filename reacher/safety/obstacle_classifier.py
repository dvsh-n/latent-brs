...
DEFAULT_DATA_PATH = "reacher/Haoran_obs_data/obstacle_data/obstacle_classifier_data.pt"
DEFAULT_OUT_DIR = "reacher/safety/obs_net"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Fraction of each class reserved for conformal calibration when the dataset has no calibration_idx.",
    )
...
def split_stratified_train_val_cal(
    ...
    *,
    ...
    if validation_frac + calibration_frac >= 1.0:
        raise ValueError("--validation-frac + --calibration-frac must be less than 1.")
    ...
    for label in sorted(np.unique(labels).tolist()):
        class_idx = np.flatnonzero(labels == label).astype(np.int64)
        rng.shuffle(class_idx)
        if class_idx.shape[0] < 3:
            raise ValueError(f"Need at least three samples for class {label}.")
        val_count = max(1, int(np.floor(validation_frac * class_idx.shape[0])))
        cal_count = max(1, int(np.floor(calibration_frac * class_idx.shape[0])))
        if val_count + cal_count >= class_idx.shape[0]:
            raise ValueError(f"Split fractions leave no training samples for class {label}.")
        val_parts.append(class_idx[:val_count])
        cal_parts.append(class_idx[val_count : val_count + cal_count])
        train_parts.append(class_idx[val_count + cal_count :])
...
def main() -> None:
    ...
    if "train_idx" in dataset and "calibration_idx" in dataset:
        ...
        split_source = "dataset_train_idx_and_calibration_idx"
        ...
        source_train_idx = np.concatenate((train_idx, val_idx), axis=0).astype(np.int64)
        split_source = "generated_stratified_train_val_cal"
