"""Latent Policy Barrier utilities for Reacher planning."""
...
class ReacherLPBBarrier:
    ...
    def nearest_distance(self, state: torch.Tensor) -> torch.Tensor:
        ...
        distances = torch.cdist(query_flat, self.prototypes)
    ...
    def state_penalty(self, state: torch.Tensor) -> torch.Tensor:
        ...
        nearest_sq = diff.square().sum(dim=-1).min(dim=-1).values
        distance = torch.sqrt(nearest_sq.clamp_min(1e-12))
        excess = torch.relu(distance - self.scaled_threshold)
        return excess.square().reshape(original_shape)
