"""Closed-loop latent HJ safety filter for reacher experiments."""
...
from reacher.safety.latent_env import load_latent_safety_components
from reacher.safety.train_pyhj_reacher import build_policy
...
class ReacherHJSafetyFilter:
    ...
    def __init__(
        ...
        *,
        ...
        if self.markov_deriv != 1:
            raise ValueError(f"Reacher safety filter expects markov_deriv=1, got {self.markov_deriv}.")
        self.history_len = 2
    ...
    def current_state(self) -> torch.Tensor:
        ...
        current = self.latent_history[-1]
        previous = self.latent_history[-2] if len(self.latent_history) >= 2 else None
        state = ilqr_base.make_markov_state(current, previous).to(self.device, dtype=torch.float32)
