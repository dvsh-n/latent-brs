#!/usr/bin/env python3
"""Train latent-safety/PyHJ avoid-DDPG on the reacher latent safety env."""
...
from reacher.safety.latent_env import ReacherLatentSafetyEnv, load_latent_safety_components  # noqa: E402

DEFAULT_CACHE_PATH = "reacher/safety/cache/reacher_latent_safety_classifier_train_tanh.pt"
DEFAULT_RUN_ROOT = "reacher/safety/runs"
...
def default_run_dir(args: argparse.Namespace) -> Path:
    ...
    name = args.run_name or time.strftime("pyhj_reacher_%Y%m%d_%H%M%S")
    return (ROOT_DIR / DEFAULT_RUN_ROOT / name).resolve()


def make_env_factory(components: Any, args: argparse.Namespace, seed_offset: int):
    def _factory() -> ReacherLatentSafetyEnv:
        return ReacherLatentSafetyEnv(
