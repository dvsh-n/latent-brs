...
DEFAULT_BLOCK_POSITION_SUCCESS_THRESHOLD = 5.0
DEFAULT_BLOCK_YAW_SUCCESS_THRESHOLD = 0.25
...
def block_pose_components(current_block_pose: np.ndarray, goal_block_pose: np.ndarray) -> tuple[float, float, float]:
    current = current_block_pose.astype(np.float64)
    goal = goal_block_pose.astype(np.float64)
    position_distance = float(np.linalg.norm(current[:2] - goal[:2]))
    yaw_error = abs(angle_diff(float(current[2]), float(goal[2])))
    mixed_pose_distance = float(np.linalg.norm([current[0] - goal[0], current[1] - goal[1], yaw_error]))
    return position_distance, yaw_error, mixed_pose_distance


def goal_reached(
    current_block_pose: np.ndarray,
    goal_block_pose: np.ndarray,
    threshold: float = DEFAULT_BLOCK_POSITION_SUCCESS_THRESHOLD,
    yaw_threshold: float = DEFAULT_BLOCK_YAW_SUCCESS_THRESHOLD,
) -> tuple[bool, float]:
    position_distance, yaw_error, pose_dist = block_pose_components(current_block_pose, goal_block_pose)
    if yaw_threshold is None:
        return position_distance <= threshold, pose_dist
    return position_distance <= threshold and yaw_error <= yaw_threshold, pose_dist


def legacy_goal_reached(current_block_pose: np.ndarray, goal_block_pose: np.ndarray, threshold: float = 5.0) -> tuple[bool, float]:
