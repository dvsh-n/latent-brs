#!/usr/bin/env python3
"""Evaluate Markov-state MLP-predictor LE-WM latent rollouts on one Two Room trajectory."""
...
def preprocess_pixels(pixels: np.ndarray, *, img_size: int) -> torch.Tensor:
...
def load_episode(
    ...
    *,
    ...
    with h5py.File(dataset_path, "r") as h5:
        ...
        pixels = preprocess_pixels(pixels_np, img_size=img_size)
...
def plot_latents(
    ...
    *,
    ...
    for plot_idx, (start_dim, end_dim) in enumerate(splits, start=1):
        ...
        fig.suptitle(f"Two Room Markov MLP LE-WM latent rollout episode {episode_idx}, dims {start_dim}-{end_dim - 1}")
        fig.tight_layout()
...
def main() -> None:
    ...
    action_mean, action_std = load_action_stats(dataset_path, int(args.action_dim))
    ...
    metrics = compute_metrics(true_latents, pred_latents, history_size=int(args.history_size))
    metrics.update(
        {
