...
print_usage() {
  cat <<'EOF'
Usage:
  bash run_ogbench_episode_sweeps.sh
  bash run_ogbench_episode_sweeps.sh --episodes 0 1 2 3
  bash run_ogbench_episode_sweeps.sh --start 0 --end 9
...
The script runs the OGBench cube MPPI/SLS fixed-grasp planner for every episode index.
...
mkdir -p "${LOG_DIR}"
SWEEP_ROOT="${SCRIPT_DIR}/sweeps_ogbench"
mkdir -p "${SWEEP_ROOT}"

if [[ ${RUN_FOREGROUND} -eq 0 && -z "${OGBENCH_SWEEP_DETACHED:-}" ]]; then
  TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
  LOG_PATH="${LOG_DIR}/ogbench_sweep_${TIMESTAMP}.log"
  nohup env OGBENCH_SWEEP_DETACHED=1 bash "$0" --foreground "${ORIGINAL_ARGS[@]}" > "${LOG_PATH}" 2>&1 &
...
echo "Running OGBench planner sweeps from ${SCRIPT_DIR}"
...
mkdir -p "${OGBENCH_OUT_DIR}"

SNAPSHOT_TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
SNAPSHOT_DIR="${SWEEP_ROOT}/ogbench_launch_snapshot_${SNAPSHOT_TIMESTAMP}"
mkdir -p "${SNAPSHOT_DIR}/ogbench_cube"
...
echo
echo "All OGBench planner sweeps completed."
