#!/usr/bin/env python3
"""Run real-rope latent iLQR MPC on real hardware with safety checks."""
...
DEFAULT_CONFIG = ROOT / "rope" / "plan" / "plan_ilqr_mpc_real.yaml"
DEFAULT_MODEL_DIR = ROOT / "rope" / "models" / "mlpdyn_ft_real"
DEFAULT_OUT_DIR = ROOT / "rope" / "plan" / "ilqr_mpc_real"
...
def save_hdf5_rollout(
    ...
    *,
    ...
    with h5py.File(path, "w") as h5:
        ...
        h5.attrs["source"] = "rope/plan/plan_ilqr_mpc_real.py"
        h5.attrs["hardware"] = not bool(args.dry_run)
        h5.attrs["trajectory_type"] = "latent_ilqr_mpc_stepwise"
        ...
        h5.attrs["action_semantics"] = "clipped_planned_task_delta_from_mpc"
...
def main() -> None:
    ...
    try:
        ...
        try:
            with tqdm(range(args.max_mpc_steps), desc="Real MPC steps", unit="step") as progress:
                for step_idx in progress:
                    ...
                    _, u_plan, solve_time, n_iters, plan_cost = solver.solve(
                        current_state.detach().cpu().numpy().astype(np.float64),
                        goal_state.detach().cpu().numpy().astype(np.float64),
                    )
                    attempted_mpc_step = {
                    ...
                    if args.optimize_3d_task_actions:
                        assert isinstance(dynamics, RealTaskDynamicsTorch)
                        raw_task_action = dynamics.normalized_to_raw(u_plan[0])
                        debug_clipped_action = np.clip(
                            raw_task_action,
                            np.asarray(args.task_action_min_m, dtype=np.float64).reshape(3),
                            np.asarray(args.task_action_max_m, dtype=np.float64).reshape(3),
                        )
                        ...
                        raw_control = dynamics.normalized_to_raw(u_plan[0])
                        attempted_mpc_step.update(
                            {
                        ...
                        raw_control, q_target, duration = plan_control_step(dynamics, qpos, u_plan[0], args)
                    ...
                    rows.append(
                        {
                            ...
                            "u_plan": u_plan[0].astype(np.float32),
