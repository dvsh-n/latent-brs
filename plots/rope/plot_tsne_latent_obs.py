#!/usr/bin/env python3
"""Plot rope obstacle-dataset latents and safe/unsafe rollouts in a t-SNE map."""
...
DEFAULT_OUT_PATH = REPO_ROOT / "plots" / "rope" / "tsne_latent_obs.png"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--perplexity", type=float, default=100.0)
    parser.add_argument("--learning-rate", type=str, default="auto")
    parser.add_argument("--max-iter", type=int, default=1500)
    parser.add_argument("--max-samples", "--max-obstacle-samples", dest="max_samples", type=int, default=None)
...
def interpolate_into_tsne_space(
    ...
    *,
    ...
    k = min(max(1, int(neighbor_count)), train_latents.shape[0])
    nbrs = NearestNeighbors(n_neighbors=k).fit(train_latents)
    distances, indices = nbrs.kneighbors(query_latents)
    weights = 1.0 / np.maximum(distances, 1e-8)
    weights = weights / np.sum(weights, axis=1, keepdims=True)
    return np.einsum("nk,nkd->nd", weights, train_xy[indices])
...
def fit_tsne(latents: np.ndarray, args: argparse.Namespace) -> np.ndarray:
    ...
    learning_rate = "auto" if str(args.learning_rate) == "auto" else float(args.learning_rate)
    tsne = TSNE(
        n_components=2,
...
def draw_trajectory(ax: plt.Axes, xy: np.ndarray, *, color: str, label: str) -> None:
    ax.plot(xy[:, 0], xy[:, 1], color=color, linewidth=2.2, alpha=0.95, label=label, zorder=4)
    ax.scatter(xy[:, 0], xy[:, 1], s=16.0, color=color, edgecolors="white", linewidths=0.35, zorder=5)
    ax.scatter(xy[0, 0], xy[0, 1], s=70.0, marker="o", color=color, edgecolors="black", linewidths=0.8, zorder=6)
    ax.scatter(xy[-1, 0], xy[-1, 1], s=90.0, marker="*", color=color, edgecolors="black", linewidths=0.8, zorder=6)
...
def main() -> None:
    ...
    safe_scaled = scaler.transform(safe_embeddings)
    unsafe_scaled = scaler.transform(unsafe_embeddings)

    dataset_xy = fit_tsne(dataset_scaled, args)
    safe_xy = interpolate_into_tsne_space(dataset_scaled, dataset_xy, safe_scaled, neighbor_count=int(args.neighbor_count))
    unsafe_xy = interpolate_into_tsne_space(dataset_scaled, dataset_xy, unsafe_scaled, neighbor_count=int(args.neighbor_count))

    plt.rcParams.update({"font.size": 9, "axes.titlesize": 10, "axes.labelsize": 9, "legend.fontsize": 8})
    fig, ax = plt.subplots(figsize=(6.8, 5.4), dpi=int(args.dpi))
    ...
    draw_trajectory(ax, safe_xy, color="#16833a", label="safe rollout")
    draw_trajectory(ax, unsafe_xy, color="#c62828", label="unsafe rollout")
    ax.set_title("Rope latent t-SNE fitted on both obstacle-data labels")
    ...
    metadata = {
        ...
        "trajectory_projection": "standardize_with_dataset_stats_then_inverse_distance_weighted_nearest_dataset_tsne_neighbors",
    ...
    print(f"Encoded dataset samples: {dataset_indices.shape[0]} ({np.sum(dataset_labels == 0)} label 0, {np.sum(dataset_labels == 1)} label 1)")
