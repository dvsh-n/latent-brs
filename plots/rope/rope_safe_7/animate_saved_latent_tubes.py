#!/usr/bin/env python3
"""Animate saved Rope SLS latent tubes from tube_data.npz."""
...
def render_animation(args: argparse.Namespace, dims: list[int], out_path: Path) -> Path:
    ...
    try:
        for frame_step in range(executed.shape[0]):
            ...
            writer.append_data(frame[:, :, :3].copy())
            plt.close(fig)
    finally:
        writer.close()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--ellipsoid-path",
        type=Path,
        default=Path("rope/eval/latent_ellipsoid_noshadow/latent_ellipsoid.npz"),
    )
...
def main() -> None:
    ...
    for suffix, dims in specs:
        out_path = out_dir / f"rope_animated_latent_tubes_{suffix}_start_{args.start_step:03d}_stride_{args.plan_stride:03d}.mp4"
        saved = render_animation(args, dims, out_path)
        print(f"Saved latent tube animation to {saved}")
